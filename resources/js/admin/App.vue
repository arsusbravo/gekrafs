...
const allNavigation = [
    ...
    { name: 'Users', to: { name: 'users.index' }, match: 'users.', adminOnly: true, icon: 'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z' },
];
    ...
    <div class="flex min-h-full">
        ...
        >
            ...
            <div class="border-t border-white/10 p-4">
                <RouterLink :to="{ name: 'account' }" class="-mx-2 block rounded-lg px-2 py-1 hover:bg-white/10" :class="{ 'bg-white/10': route.name === 'account' }">
                    <p class="truncate text-sm font-medium text-white">{{ currentUser.name }}</p>
                    <p class="truncate text-xs text-brand-300">{{ currentUser.email }}</p>
                    <p class="mt-0.5 text-xs font-medium text-accent-400">{{ isAdmin() ? 'Administrator' : 'User' }} · My account</p>
                </RouterLink>
